...
class WebSocketService {
  ...
  final OnConnectionChange
      onConnectionChange; // Callback khi trạng thái kết nối thay đổi
  ...
  void connect() {
    stompClient = StompClient(
      config: StompConfig(
        url:
            'ws://192.168.1.13:8080/notifications/websocket', // URL WebSocket của server
  ...
  void _onConnect(StompFrame frame) {
    ...
    stompClient.subscribe(
      destination:
          '/user/$userId/notifications/websocket', // Kênh nhận thông báo của user
      callback: (StompFrame frame) {
        if (frame.body != null) {
          try {
            final Map<String, dynamic> data =
                jsonDecode(frame.body!); // Chuyển đổi JSON thành Map
